  ...
  return (
    <footer className="border-t border-border-glass">
      <div className="mx-auto max-w-[1440px] px-4 py-16 md:px-8 lg:px-12">
        <div className="grid grid-cols-2 gap-8 md:grid-cols-3 md:max-w-2xl">
          {COLS.map((col) => (
            <div key={col.label}>
              <div className="text-label text-text-secondary">{col.label}</div>
              <ul className="mt-4 flex flex-col gap-3">
                {col.links.map((l) => (
                  <li key={l.href}>
                    <Link
                      href={l.href}
                      className="text-sm text-text-primary transition-colors hover:text-blue-300"
                    >
                      {l.label}
                    </Link>
                  </li>
        ...
        <div className="mt-12 flex flex-col gap-3 border-t border-border-glass pt-6 text-xs text-text-secondary md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <a
              ...
              className="text-text-primary transition-colors hover:text-blue-300"
            >
              Simnetiq Ltd
            </a>
